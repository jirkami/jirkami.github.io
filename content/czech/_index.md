---
# Banner
banner:
  title: "Stabilní, bezpečná a efektivní IT infrastruktura"
  content: "Více než 20 let zkušeností se systémovou správou, automatizací a cloud technologiemi. Pomáhám firmám budovat a spravovat spolehlivou IT infrastrukturu."
  image: "/images/banner.png"
  button:
    enable: true
    label: "Kontaktujte mě"
    link: "contact"

# Features
features:
  - title: "Serverová infrastruktura"
    image: "/images/service-1.png"
    content: "Komplexní správa serverové infrastruktury s důrazem na bezpečnost, výkon a dostupnost."
    bulletpoints:
      - "Správa Linux serverů (Debian, Ubuntu, CentOS) - zkušenosti od roku 2002"
      - "Windows Server - Active Directory, Hyper-V, IIS"
      - "Virtualizace - VMware, Proxmox, Hyper-V"
      - "Monitoring a alerting - Prometheus, Grafana, Zabbix, Nagios"
      - "Automatizace nasazení - Ansible, Terraform, Packer"
      - "Kontejnerizace - Docker, Kubernetes"
    button:
      enable: true
      label: "Více informací"
      link: "services"

  - title: "Cloud a síťová infrastruktura"
    image: "/images/service-2.png"
    content: "Návrh, implementace a správa cloudových řešení a síťové infrastruktury pro maximální efektivitu a bezpečnost."
    bulletpoints:
      - "AWS - EC2, S3, RDS, Lambda, CloudFormation, správa nákladů"
      - "Azure a Exoscale - migrace, správa zdrojů"
      - "Síťový design - VPN, VLAN, segmentace"
      - "Správa síťového HW - Ubiquiti, Mikrotik, Cisco, HPE/Aruba"
      - "Firewall a bezpečnost - iptables, pfSense, security audity"
      - "VoIP - Asterisk, FreePBX"
    button:
      enable: true
      label: "Zjistit více"
      link: "services"

  - title: "Bezpečnost a compliance"
    image: "/images/service-3.png"
    content: "Zabezpečení IT infrastruktury podle moderních standardů a best practices."
    bulletpoints:
      - "Hardening účtů a zařízení podle CIS Benchmarks"
      - "Konfigurace firewallů a bezpečnostní audity"
      - "Správa Active Directory a skupinových politik"
      - "Compliance audity a dokumentace"
      - "Zálohování a disaster recovery plány"
      - "SSL/TLS certifikáty a bezpečná komunikace"
    button:
      enable: false
      label: ""
      link: ""
---
